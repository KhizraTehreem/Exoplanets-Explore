...
// Function to show a thought bubble on one of the exoplanets
function showThoughtBubble(planetElement) {
    // Create the thought bubble element
    const bubble = document.createElement('div');
    bubble.innerText = 'Touch or click me!';
    
    // Apply styles directly to the bubble
    ...
    bubble.style.zIndex = '10'; // Ensure it's on top of other elements
    
    // Add animation effect
    bubble.style.animation = 'bubbleAnimation 2s infinite';
    
    // Position the bubble relative to the planet element
    bubble.style.top = (planetElement.offsetTop - 40) + 'px';  // Adjust to position above planet
    bubble.style.left = (planetElement.offsetLeft + 20) + 'px';  // Adjust for horizontal positioning
    
    // Append the bubble to the body
    document.body.appendChild(bubble);

    // Remove the bubble after 5 seconds (optional)
    setTimeout(() => {
        bubble.remove();
    }, 5000);  // Remove after 5 seconds
}

// Add the animation directly in JavaScript
...
// Example usage: Show the bubble when the planet image is loaded
document.getElementById('planet-image').onload = function() {
    const planetElement = document.getElementById('planet-image');
    showThoughtBubble(planetElement);
};
